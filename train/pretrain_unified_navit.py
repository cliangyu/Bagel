...
def main():
    ...
    if resume_from is not None:
        checkpoint_format = ShardedCheckpointManager.detect_checkpoint_format(resume_from)
        logger.info(f"Detected checkpoint format: {checkpoint_format}")
        
        if checkpoint_format == "unknown":
            logger.warning(f"Unknown checkpoint format at {resume_from}, attempting HF format loading")
    ...
    if resume_from is not None and checkpoint_format == "sharded":
        ...
        fsdp_model, ema_model, _, _, _, _ = ShardedCheckpointManager.load_sharded_checkpoint(
